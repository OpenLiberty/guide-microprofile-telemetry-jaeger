...
Verify that there are two spans from `inventory` service and one span from `system` service:
...
The MicroProfile Telemetry feature has been enabled tracing of all REST endpoints by default in the previous section. To further control and customize traces, use the `@Traced` annotation to enable particular methods. You can also inject a custom `Tracer` object to create and customize spans.
...
You can also create new spans by annotating methods with a `@WithSpan` annotation.
...
Annotate the `add()` method with the [hotspot=addWithSpan file=1]`@WithSpan` annotation and the `hostname` parameter with the [hotspot=spanAttribute file=1]`@SpanAttribute` annotation to indicate that it is part of the trace.

To customize a span name, you can annotate the `list()` method by the [hotspot=listInventory file=1]`@WithSpan` annotation with a value.
