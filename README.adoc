...
Specify the [hotspot=service file=2]`otel.service.name` property with the service name, set the [hotspot=disabled file=2]`otel.sdk.disabled` property with `false` to enable tracing, and specify the [hotspot=exporter file=2]`otel.traces.exporter` property with the `jaeger` for using Jaeger tracing system.
...
Simiarly, enable the [hotspot=otel file=3]`otel` properties for the `inventory` service.

For more information on these properties, see https://github.com/open-telemetry/opentelemetry-specification/blob/main/specification/sdk-environment-variables.md[OpenTelemetry configuration documentation^].
...
Navigate to the http://localhost:9080/system/properties endpoint.

A span for this request will be automatically created by MicroProfile Telemetry. Check for the span in your chosen exporter’s endpoint like following:
...
Third-party APIs must be made visible for your application in the [hotspot=thirdParty file=0]`server.xml`:
...
The OpenTelemetry API and OpenTelemetry Instrumentation Annotations must be provided as dependencies to your build path. Add the the [hotspot=opentelemetry file=1] `pom.xml` file.
