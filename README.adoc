...
The [hotspot=mpTelemetry file=1]`mpTelemetry` feature is now enabled the `server.xml` of the `inventory` service.
...
The MicroProfile properties file specifies the [hotspot=service file=2]`otel.service.name` property with the `system` service name and sets the [hotspot=disabled file=2]`otel.sdk.disabled` property with `false` to enable tracing.
...
Before the [hotspot=manager hotspot=getSystem file=0]`InventoryManager` calls the `system` service, the `spanBuilder()` and `startSpan()` Tracer APIs create and start a span called [hotspot=getPropertiesSpan file=0]`GettingProperties`.
