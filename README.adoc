...
The `makeCurrent()` call returns a `Scope`. It's very important to always close the `Scope`, which stops the span being current and makes the previous span current again. 
The recommended way to do this is to use a [hotspot=scope file=0]`try-with-resources` block, which makes sure that the `Scope` is automatically closed at the end of the block.
