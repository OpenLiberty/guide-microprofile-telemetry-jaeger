...
image::mptelemetry_result_items.png[Compare traces by selecting result items,align="center"]
{empty} +
...
To customize a span name, you can annotate the `list()` method by the [hotspot=listInventory file=0]`@WithSpan` annotation with a value.
...
Navigate your browser to the http://localhost:9081/inventory/systems/localhost and http://localhost:9081/inventory/systems URLs.
