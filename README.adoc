...
When calling makeCurrent() , it returns a Scope object. You need to call Scope.close() when you want the span to stop being the current span. The easiest way to do this is to call makeCurrent() in a try-with-resources block, which ensure the Scope is automatically closed at the end of the block.
