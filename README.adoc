...
The diagram above depicts the architecture of the guide. Please note that the blue solid lines indicate the services included in the guide, while the black dotted line represents services that can be added to the system.
...
The [hotspot=addMethod file=0]`add()` and [hotspot=listMethod file=0]`list()` methods are annotated with the [hotspot=addWithSpan hotspot=listWithSpan file=0]`@WithSpan` annotation. The OpenTelemetry instrumentation provides a new span for each method. You can now collect and trace the spans across different services. After creating a span, you have the option to include desired parameters with their values in the span by using the [hotspot=spanAttribute file=0]`@SpanAttribute` annotation. For example, you can annotate the [hotspot=spanAttribute file=0]`host` parameter with the [hotspot=spanAttribute file=0]`@SpanAttribute` annotation and assign [hotspot=spanAttribute file=0]`hostname` as its attribute name. This allows you to trace the parameters within the [hotspot=addMethod file=0]`add` span.
