...
= Enabling distributed tracing in microservices by using the OpenTelemetry observability framework 

[.hidden]
NOTE: This repository contains the guide documentation source. To view the guide in published form, view it at the https://openliberty.io/guides/{projectid}.html[Open Liberty website].

Distributed tracing helps teams keep track of requests between microservices. MicroProfile Telemetry adopts OpenTelemetry tracing, allowing developers to observe requests across their distributed systems.
...
== What you'll learn
The use of a microservices architecture can increase the difficulty of understanding how services depend on or affect other services. Therefore, It makes it harder to identify the source of latency or inaccuracies.

One way to increase the observability of an application is by emitting traces. https://opentelemetry.io/[OpenTelemetry^] is a set of APIs, SDKs, tooling, and integrations designed to create and manage telemetry data such as traces, metrics, and logs. MicroProfile Telemetry adopts OpenTelemetry, so your applications can benefit from both manual and automatic traces.

Traces represent requests, which can contain multiple operations or spans. Each span comprises a name, time-related data, log messages, and metadata that describes what occurred during a transaction. Spans are associated with a context, which identifies the request within which the span occurred. Developers can then follow a single request between services through a potentially complex distributed system. Exporters send the data that MicroProfile Telemetry 1.0 collects to Jaeger so you can visualize and monitor the generated spans.
...
Before you begin, deploy the Jaeger all-in-one executable file to start the Jaeger tracing system. The Jaeger all-in-one executable file is configured for quick local testing. Jaeger 1.46 or later is recommended. You can find information about the Jaeger server, and the instructions for starting the all-in-one executable file in the https://www.jaegertracing.io/docs/latest/getting-started[Jaeger documentation].
...
When you visit this endpoint, make two GET HTTP requests, one to the `system` service and another to the `inventory` service. Both of these requests are configured to be traced, so a new trace is recorded in Jaeger. To view the traces, go to the http://localhost:16686 URL.
endif::[]
...
When you visit this endpoint, make two GET HTTP requests, one to the ***system*** service and another to the ***inventory*** service. Both of these requests are configured to be traced, so a new trace is recorded in Jaeger. To view the traces, click the following button to visit the Jaeger service:

::startApplication{port="16686" display="external" name="Visit Jaeger service" route="/"}
endif::[]

You can view the traces for the `system` or `inventory` services under the **Search** tab. If you see only the **jaeger-query** option that is listed in the drop-down, wait a little longer and refresh the page to see the application services.
...
The [hotspot=addMethod file=0]`add()` and [hotspot=listMethod file=0]`list()` methods are annotated with the [hotspot=addWithSpan hotspot=listWithSpan file=0]`@WithSpan` annotation. The OpenTelemetry instrumentation provides a new span for each method. You can now collect and trace the spans across different services. After creating a span, you have the option to include desired parameters with their values in the span by using the [hotspot=spanAttribute file=0]`@SpanAttribute` annotation. For example, the [hotspot=spanAttribute file=0]`host` parameter is assigned [hotspot=spanAttribute file=0]`hostname` as its attribute name using the [hotspot=spanAttribute file=0]`@SpanAttribute` annotation, which gives the benefit of tracing the parameters within the [hotspot=addMethod file=0]`add` span.
...
Before the [hotspot=manager hotspot=getSystem file=0]`InventoryManager` calls the `system` service, it creates and starts a span called the [hotspot=getPropertiesSpan file=0]`GettingProperties` span by using the `spanBuilder()` and `startSpan()` Tracer APIs.
...
The `makeCurrent()` call returns a `Scope`. It is important to always close the `Scope`, which stops the span from being current and makes the previous span current again. The recommended way to do this is to use a [hotspot=try file=0]`try-with-resources` block, which makes sure that the `Scope` is automatically closed at the end of the block.
...
Manually verify the traces by inspecting them on the Jaeger server. You will find some tests included to assess the basic functionality of the services. If any of the tests fail, it might indicate a potential bug in the code.
