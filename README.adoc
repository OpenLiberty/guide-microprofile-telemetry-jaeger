...
When starting a span, you will need to end it by calling end() on the span. If you don't end a span, it won't be recorded at all and won't show up in Jaeger. This code ensures that end() is always called by including it in a finally block.
...
When calling makeCurrent() , it returns a Scope object. You must always call Scope.close() in order for the span to stop being the current span. The easiest way to do this is to call makeCurrent() in a try-with-resources block, which ensures the Scope is automatically closed at the end of the block.
