...
The [hotspot=microprofile hotspot=mpTelemetry file=1]`mpTelemetry` feature is now enabled in the `server.xml` of the `inventory` service.
...
// static guide instructions:
ifndef::cloud-hosted[]
Finally, stop the `Jaeger` service that you started in the **Additional prerequisites** section /*this is non-cloud hosted test. */
endif::[]

// cloud-hosted guide instructions:
ifdef::cloud-hosted[]
Finally, stop the ***Jaeger*** service that you started in the previous step /*this is cloud hosted test. */
```bash
docker stop jaeger
docker rm jaeger
```
