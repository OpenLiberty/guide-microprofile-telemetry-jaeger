...
The [hotspot=addMethod file=0]`add()` and [hotspot=listMethod file=0]`list()` methods are annotated with the [hotspot=addWithSpan hotspot=listWithSpan file=0]`@WithSpan` annotation, which can accept an optional parameter functioning as the span's name. In this example, the span name has been designated as [hotspot=addWithSpan file=0]`My Span` for the [hotspot=addMethod file=0]`add()` method. If you prefer to use the default span name, as demonstrated in the case of the [hotspot=listMethod file=0]`list()` method, an appropriate default name will be automatically created through instrumentation. The OpenTelemetry instrumentation provides a new span for each method. You can now collect and trace the spans across different services. After creating a span, you have the option to include desired parameters with their values in the span by using the [hotspot=spanAttribute file=0]`@SpanAttribute` annotation. For example, the [hotspot=spanAttribute file=0]`host` parameter is assigned [hotspot=spanAttribute file=0]`hostname` as its attribute name using the [hotspot=spanAttribute file=0]`@SpanAttribute` annotation, which gives the benefit of tracing the parameters within the [hotspot=addMethod file=0]`add` span.
...
Click the `My Span` span and its `Tags`. You can see the `hostname` tag with the `localhost` value that is created by the `@SpanAttribute` annotation.
