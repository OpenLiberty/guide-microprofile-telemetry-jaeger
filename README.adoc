...
The [hotspot=addMethod file=0]`add()` and [hotspot=listMethod file=0]`list()` methods are annotated with the [hotspot=addWithSpan hotspot=listWithSpan file=0]`@WithSpan` annotation. The OpenTelemetry instrumentation provides a new span for each method. You can now collect and trace the spans across different services. After creating a span, you have the option to display a desired parameter in the trace/span by using the @SpanAttribute annotation. For example, you can annotate the [hotspot=spanAttribute file=0]`host` parameter with the [hotspot=spanAttribute file=0]`@SpanAttribute` annotation and assign it the attribute name [hotspot=spanAttribute file=0]`hostname`. This allows you to trace and monitor the parameters within the [hotspot=addMethod file=0]`add()` span.
