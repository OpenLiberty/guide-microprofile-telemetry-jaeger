...
Annotate the [hotspot=addWithSpan file=0]`add()` and [hotspot=listWithSpan file=0]`list()` methods with the `@WithSpan` annotation. You can annotate the [hotspot=spanAttribute file=0]`host` parameter with the `@SpanAttribute` annotation with a customized value to indicate that it is part of the trace.
