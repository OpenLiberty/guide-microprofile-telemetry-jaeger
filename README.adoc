...
Before the [hotspot=manager hotspot=getSystem file=0]`InventoryManager` to call the `system` service, create and start a span called `GettingProperties` by using the `spanBuilder()` and `startSpan()` Tracer APIs.
...
Use the [hotspot=addEvent1 hotspot=addEvent2 file=0]`addEvent()` Span API to create an event when the properties are received and fails to get the properties from the `system` service with `hostname`. Use the [hotspot=end file=0]`end()` Span API to mark the span completed.
