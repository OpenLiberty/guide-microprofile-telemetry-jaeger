...
Annotate the `add()` method with the `@WithSpan` annotation and the `hostname` parameter with the `@SpanAttribute annotation` annotation to indicate that it is part of the trace.

To customize a span name, you can annotate the `list()` method by the `@WithSpan` annotation with a value.
