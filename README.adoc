...
The [hotspot=listMethod file=0]`list()` and [hotspot=addMethod file=0]`add()` methods are annotated with the [hotspot=addWithSpan hotspot=listWithSpan file=0]`@WithSpan` annotation, which can accept an optional parameter that functions as the span name. In this example, the default span name assigned to the [hotspot=listMethod file=0]`list()` method is automatically generated through instrumentation. You can also specify a custom span name. For example, [hotspot=addWithSpan file=0]`Inventory Manager Add` is specified as the span name for the [hotspot=addMethod file=0]`add()` method. The OpenTelemetry instrumentation provides a new span for each method. You can now collect and trace the spans across different services. 
